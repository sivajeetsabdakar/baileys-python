...
| Groups | Metadata/create/participants/invites/settings | Partial | Product group metadata/create/leave/subject/description/participants/invite/settings APIs and parsers are wired with offline tests; metadata, invite lookup, group text send, announcement setting change/revert, invite revoke, subject/description set/revert, participant promote, and participant demote are live-proven. Participant add and temporary group create/leave are account-gated in the current live account; participant add returns `account_reachout_restricted`, and product APIs now raise explicit IQ errors for server-side rejections. The current live test group is single-member until restored, so participant-dependent group mutation probes are blocked by account state. |
...
| Tooling | Live test harness | Partial | Product scripts cover QR pairing, pairing-code linking, saved reconnect, inbound 1:1/group proof, timed soak checks, text/image/media sends, reaction/edit/delete/location/contact/poll/pin, Phase 5 read/write/mutation probes, and app-state key diagnostics; pytest-style live suite still needed. |
