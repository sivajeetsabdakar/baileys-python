...
| Chats | Presence/status/profile/privacy/blocklist/chat modify | Partial | Product presence, profile status/picture, privacy, blocklist, and block APIs are wired with offline coverage; live read-only profile/privacy/blocklist/on-whatsapp, all supported presence write states, profile status update, and profile picture update are proven. Profile name and chat patches need encrypted app-state patch support before live parity. |
