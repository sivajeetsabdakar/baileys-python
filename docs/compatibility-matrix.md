...
| Outbound | `sendMessage` text | Partial | Product `send_message`/`sendMessage` supports direct and USync fanout paths; direct session bootstrap and sparse-USync fallback are implemented and tested. Fresh live proof is still needed for third-party peer states where device discovery returns no peer devices. |
...
| Messages | Text/extended text | Partial | Product content builders cover text, extended text, mentions, quote keys, and forwarded markers with offline tests; live group and self text sends are proven. |
...
| Media | Image upload/send/download | Partial | Product media connection, encrypt/upload, image message generation, send, and download/decrypt helpers are wired; rerun the live `--download` probe after relinking saved auth. |
...
| Chats | Presence/status/profile/privacy/blocklist/chat modify | Partial | Product chat, presence, profile status/name/picture, privacy, blocklist, and block APIs are wired with offline coverage; live read-only and write-side checks need a freshly linked saved session. |
...
| Groups | Metadata/create/participants/invites/settings | Partial | Product group metadata/create/leave/subject/description/participants/invite/settings APIs and parsers are wired with offline tests; metadata/invite and participant/update live checks need a freshly linked saved session. |
