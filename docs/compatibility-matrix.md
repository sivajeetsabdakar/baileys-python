...
| Messages | Quote/mention/forward/location/contact/reaction/pin/poll/edit/delete | Partial | Quote keys, mentions, reactions, edit, delete, location, contact, and contact-list builders are offline-tested; reaction add/remove, edit, and delete are live-proven with ACK. Pins, polls, and live quote/location/contact sweeps remain. |
...
| Chats | Presence/status/profile/privacy/blocklist/chat modify | Partial | Product presence, profile status/picture, privacy, blocklist, block APIs, and encrypted app-state patch writes are wired with offline coverage; live read-only profile/privacy/blocklist/on-whatsapp, all supported presence write states, profile status update, and profile picture update are proven. Profile name and chat patches need live app-state key material in the saved session before live proof. |
| History | History sync/app-state/LTHash/MAC validation | Partial | Key derivation, app-state key-share ingestion, LT hash update, encrypted patch generation, patch MAC generation, app-state snapshot request, external app-state blob download/decrypt, snapshot metadata decode, snapshot mutation decrypt/MAC validation, and blocked-key persistence exist; patch-sequence application, history sync, and peer key-share response handling still need completion. |
...
| Groups | Metadata/create/participants/invites/settings | Partial | Product group metadata/create/leave/subject/description/participants/invite/settings APIs and parsers are wired with offline tests; metadata, invite lookup, group text send, announcement setting change/revert, invite revoke, participant promote, and participant demote are live-proven. Create/leave, add/remove, subject/description, and additional settings still need targeted live sweeps. |
...
| Tooling | Live test harness | Partial | Product scripts cover QR pairing, pairing-code linking, saved reconnect, inbound 1:1/group proof, timed soak checks, text/image/media sends, reaction/edit/delete, Phase 5 read/write/mutation probes, and app-state key diagnostics; pytest-style live suite still needed. |
