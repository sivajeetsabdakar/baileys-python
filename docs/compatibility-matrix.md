...
| History | History sync/app-state/LTHash/MAC validation | Partial | Key derivation, app-state key-share ingestion, LT hash update, encrypted patch generation, and patch MAC generation exist; full snapshot/patch decode, history sync, and live key acquisition still need completion. |
...
| Tooling | Live test harness | Partial | Product scripts cover QR pairing, pairing-code linking, saved reconnect, inbound 1:1/group proof, timed soak checks, text/image/media sends, reaction/edit/delete, and Phase 5 read/write/mutation probes; pytest-style live suite still needed. |
