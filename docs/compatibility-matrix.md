...
| Business | Profile/catalog/products/orders | Partial | Business profile update, catalog, collections, order details, product create/update/delete nodes, product image media preparation, cover-photo update/remove, aliases, and parsers are wired with offline tests. Current live account returns `item-not-found` for self catalog; enabled-account live proof remains. |
