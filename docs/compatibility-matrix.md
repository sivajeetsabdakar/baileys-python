...
| Outbound | Privacy tokens/peer data operations | Partial | Trusted-contact token storage, expiry checks, issuance JID resolution, and `tctoken` node construction are ported with offline tests. Full peer-data operations and broader live proof remain. |
...
| Media | Image upload/send/download | Done | Product media connection, encrypt/upload, image message generation, send, and download/decrypt helpers are wired and live-proven for image. |
...
| Chats | Presence/status/profile/privacy/blocklist/chat modify | Partial | Product presence, profile name/status/picture, privacy, blocklist, block APIs, and encrypted app-state patch writes are wired with offline coverage; live read-only profile/privacy/blocklist/on-whatsapp, all supported presence write states, profile name/status/picture updates, chat archive/mute/pin/star patches, and app-state key-share delivery are proven. Block/unblock mutation still needs a dedicated disposable contact. |
...
| Groups | Metadata/create/participants/invites/settings | Partial | Product group metadata/create/leave/subject/description/participants/invite/settings APIs and parsers are wired with offline tests; metadata, PN mapping for LID participants, invite lookup, group text send, announcement setting change/revert, invite revoke, description set/revert with previous id, subject set/revert, participant promote, and participant demote are live-proven. Participant add and temporary group create/leave are account-gated in the current live account; participant add returns `account_reachout_restricted`, and product APIs now raise explicit IQ errors for server-side rejections. |
...
| Business | Profile/catalog/products/orders | Partial | Business profile update, catalog, collections, order details, product create/update/delete nodes, cover-photo update/remove, aliases, and parsers are wired with offline tests. Current live account returns `item-not-found` for self catalog; product image media for catalog products and enabled-account live proof remain. |
| Newsletters | MEX/newsletter APIs/events | Partial | MEX query/response helpers plus newsletter create/update/metadata/follow/unfollow/mute/subscribers/admin/owner/delete/picture/reaction/message-fetch/live-update APIs are wired with offline tests. Read-only live proof confirms account reachout timelock MEX; message capping is server-rejected for the current account/request shape, and newsletter live event processing still remains. |
