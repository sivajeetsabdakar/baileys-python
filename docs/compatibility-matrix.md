...
| Outbound | `sendMessage` text | Partial | Product `send_message`/`sendMessage` supports direct and USync fanout paths; direct 1:1 text is live-proven with ACK, and third-party USync/session assertion is live-proven for multiple peer devices. Group outbound still needs a fresh group run. |
...
| Media | Image upload/send/download | Partial | Product media connection, encrypt/upload, image message generation, send, and download/decrypt helpers are wired and live-proven for image. |
...
| Chats | Presence/status/profile/privacy/blocklist/chat modify | Partial | Product chat, presence, profile status/name/picture, privacy, blocklist, and block APIs are wired with offline coverage; live read-only profile/privacy/blocklist/on-whatsapp and presence write are proven. Profile mutations and chat patches still need safe live targets. |
...
| Groups | Metadata/create/participants/invites/settings | Partial | Product group metadata/create/leave/subject/description/participants/invite/settings APIs and parsers are wired with offline tests; metadata/invite and participant/update live checks need a safe group target. |
