...
| Media | Video/audio/document/sticker/stream/url inputs/thumbnails/reupload | Partial | Document and sticker send/download/decrypt are live-proven with generated valid samples. Video/audio need real sample fixtures; streams, URLs, thumbnails, waveform, duration extraction, retry/reupload, and broader live breadth remain. |
| Chats | Presence/status/profile/privacy/blocklist/chat modify | Partial | Product chat, presence, profile status/name/picture, privacy, blocklist, and block APIs are wired with offline coverage; live read-only profile/privacy/blocklist/on-whatsapp and all supported presence write states are proven. Profile mutations and chat patches still need safe live targets. |
...
| Groups | Metadata/create/participants/invites/settings | Partial | Product group metadata/create/leave/subject/description/participants/invite/settings APIs and parsers are wired with offline tests; metadata, invite lookup, and group text send are live-proven. Participant/update, create/leave, revoke invite, and settings mutations still need explicit safe targets. |
...
| Tooling | Live test harness | Partial | Product scripts cover QR pairing, pairing-code linking, saved reconnect, inbound 1:1/group proof, timed soak checks, text/image/media sends, reaction/edit/delete, and Phase 5 read/write probes; pytest-style live suite still needed. |
