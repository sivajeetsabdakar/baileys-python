...
| History | History sync/app-state/LTHash/MAC validation | Partial | Key derivation, app-state key-share ingestion, LT hash update, encrypted patch generation, patch MAC generation, app-state snapshot request, external app-state blob download/decrypt, snapshot metadata decode, and blocked-key persistence exist; full snapshot/patch mutation decode, history sync, and peer key-share response handling still need completion. |
