...
| Core | WAM telemetry encoder | Partial | WAM packet/header and scalar field encoding are ported with `encode_wam` / `encodeWAM` and offline vectors. Generated event/global constants and upload submission remain. |
...
| Communities | Community APIs | Partial | Product community metadata/create/linked group/leave/subject/description/participants/invite/settings/member-add/join-approval APIs, aliases, node builders, and parsers are wired with offline tests. Live proof needs an account with enabled community surfaces. |
| Business | Profile/catalog/products/orders | Partial | Business profile update, catalog, collections, order details, product create/update/delete nodes, aliases, and parsers are wired with offline tests. Current live account returns `item-not-found` for self catalog; product media and cover-photo flows remain. |
| Newsletters | MEX/newsletter APIs/events | Partial | MEX query/response helpers plus newsletter create/update/metadata/follow/unfollow/mute/subscribers/admin/owner/delete/reaction/message-fetch/live-update APIs are wired with offline tests. Read-only live proof confirms account reachout timelock MEX; message capping is server-rejected for the current account/request shape, and newsletter live event processing still remains. |
